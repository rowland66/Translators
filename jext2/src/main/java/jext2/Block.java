...
public abstract class Block {
	...
	final void setBlockNr(long blockNr) {
		this.nr = blockNr;
	}
	...
	void write(ByteBuffer buf) throws IoError {
		if (getBlockNr() == -1)
			throw new IllegalArgumentException("data structure is unregistered");

		BlockAccess.getInstance().writePartial(nr, 0, buf);
	}
	...
	abstract void read(ByteBuffer buf) throws IoError;
